...
void PredictionSystem::Start(CUserCmd *userCMD, C_BasePlayer* player)
{
	*predictionRandomSeed = MD5_PseudoRandom(userCMD->command_number) & 0x7FFFFFFF;
	...
	int buttonsChanged = userCMD->buttons ^ *reinterpret_cast<int*>(uint32_t(player) + 0x31F8);
	*reinterpret_cast<int*>(uint32_t(player) + 0x31EC) = (uint32_t(player) + 0x31F8);
	*reinterpret_cast<int*>(uint32_t(player) + 0x31F8) = userCMD->buttons;
	*reinterpret_cast<int*>(uint32_t(player) + 0x31F0) = userCMD->buttons & buttonsChanged;  //m_afButtonPressed ~ The changed ones still down are "pressed"
	*reinterpret_cast<int*>(uint32_t(player) + 0x31F4) = buttonsChanged & ~userCMD->buttons; //m_afButtonReleased ~ The ones not down are "released"
	
	if (userCMD->impulse)
		*reinterpret_cast<uint32_t*>(uint32_t(player) + 0x31FC) = userCMD->impulse;
